...
| T19 | Image model supply chain | Malicious or corrupted model | Official release file, unmodified, pinned by size and SHA-256 (test-checked), memory-mapped read-only, tensor shapes checked, LiteRT runs a data file (no code); load failures → UNAVAILABLE | Web-scraped training data (legal/ethical provenance risk, owner-accepted) |
| T20 | Screen capture | Frames leaked or kept; capture outliving consent | MediaProjection only after Android's dialog; system indicator + notification; frames only while a supported app is in front; released right after inference; no bitmaps/files/network in the capture code (test-enforced); stops with protection/shield | A rooted device or OS exploit defeats the sandbox |
