...
done or not verified. State as of version 1.6.0+7, end of Phase 7.
...
- [x] **Threat model complete:** docs/THREAT_MODEL.md.
- [x] **Permissions reviewed:** docs/GOOGLE_PLAY.md §3 (3 permissions + 2 service bindings, none removable without losing a feature).
...
- [ ] **Database tested:** JVM failure-path tests pass. The Robolectric SQLite tests have **not** been run (M37: needs `./gradlew test` on a machine with the Android SDK).
...
- [ ] **Real-device testing completed:** 3 PASS rows out of 37 (TEST_MATRIX).
