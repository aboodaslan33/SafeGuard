...
> **الحالة: المرحلة الرابعة (Phase 4).** إلى جانب فلترة DNS المحلية (المرحلة 2)
> وحماية البحث والتطبيقات (المرحلة 3)، أُضيفت **الحماية الذكية**: محرك تصنيف
> محتوى على الجهاز (نص وصور) خلف واجهة قابلة للتبديل، ومحرك قرار يجمع القواعد
> والذكاء الاصطناعي وإعدادات المستخدم (عادي / صارم / مخصص)، و«إبلاغ عن حظر
> خاطئ». نموذج النص صغير ومحدود الدقة، **ولا يوجد نموذج صور في هذا الإصدار**.
>
...
> (Flutter 114، Kotlin 173) وطبقة Android مُتحقق من ترجمتها فقط. انظر
...
|---|---|---|
| Flutter (`flutter test`) | 114 | ✅ (منها 20 للمرحلة 3 و21 للمرحلة 4) |
| Kotlin JUnit (المحرك) | 173 | ✅ (منها 45 للمرحلة 3 و83 للمرحلة 4)، شُغّلت عبر أداة JVM لأن `./gradlew test` يحتاج Android SDK |
...
التفاصيل: [`docs/PHASE_4_AI.md`](docs/PHASE_4_AI.md) (المرحلة 4، مع قياسات الأداء)،
...
- **قبل أي شيء:** بناء APK واختبار المراحل 2 و3 و4 يدويًا على جهاز حقيقي.
